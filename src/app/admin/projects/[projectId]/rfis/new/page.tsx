...
import { rfiStatusOptions, rfiVisibilityOptions } from "@/lib/admin/rfis";
import { createProjectRfiAction } from "./actions";

function todayIsoDate() {
  return new Date().toISOString().slice(0, 10);
}

export default async function NewProjectRfiPage({
  params,
  searchParams,
}: {
  ...
  const action = createProjectRfiAction.bind(null, project.id);

  return (
    <AdminShell title={`RFI for ${project.name}`} eyebrow="Messaging">
      <div className="mb-6">
        <Link
      ...
      <form action={action} className="space-y-6">
        <section className="rounded-lg border border-black/10 bg-white p-5 shadow-sm">
          <div className="mb-5 flex items-start gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold">RFI Details</h2>
          ...
          <div className="grid gap-4 md:grid-cols-[1.2fr_0.6fr_0.5fr_0.5fr]">
          ...
          <label className="mt-4 block text-sm font-semibold text-[#171717]">
            ...
            <select
              ...
              required
            >
              {rfiVisibilityOptions.map((option) => (
                <option key={option.value} value={option.value}>
                  {option.label}
                </option>
              ))}
