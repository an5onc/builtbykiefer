...
export default async function LoginPage({
  ...
  const { error, next = "/admin", notice } = await searchParams;
    ...
    <main className="flex min-h-screen items-center justify-center bg-[#151515] px-6 text-white">
      <section className="w-full max-w-md rounded-lg border border-white/10 bg-white/[0.04] p-8 text-center shadow-2xl">
        ...
        <p className="mt-4 text-sm leading-6 text-white/65">
          {demoMode
            ? "Demo mode is enabled for local walkthroughs until Supabase credentials are connected."
            : "Sign in with the approved Supabase admin account or request a passwordless email link."}
        </p>
