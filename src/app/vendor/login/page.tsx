...
import { sendVendorMagicLink, signInVendor } from "./actions";
...
export default async function VendorLoginPage({
  ...
  const { error, next = "/vendor", notice } = await searchParams;
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#151515] px-6 text-white">
      <section className="w-full max-w-md rounded-lg border border-white/10 bg-white/[0.04] p-8 text-center shadow-2xl">
        ...
        <h1 className="mt-2 text-3xl font-bold">Vendor Access</h1>
        <p className="mt-4 text-sm leading-6 text-white/65">
          Sign in with the email Kiefer Built has approved for your subcontractor or vendor account.
        </p>
        {error ? (
        ...
        <div className="mt-6 space-y-4 text-left">
          <form action={signInVendor} className="space-y-4">
          ...
          <form action={sendVendorMagicLink} className="space-y-3">
            <label className="block text-sm font-semibold text-white/80">
              Email link
              <input
                name="email"
            ...
            >
              Email Me a Vendor Sign-In Link
            </button>
          </form>
        </div>
      </section>
